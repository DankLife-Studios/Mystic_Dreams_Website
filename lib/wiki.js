...
/** Updates a category's description, parent, name, and display order. */
export async function updateWikiCategory(name, { description, parentName, newName, displayOrder } = {}) {
    ...
    if (renamed && renamed !== trimmed) {
        const conflict = await tursoQueryOne(
            "SELECT name FROM wiki_categories WHERE name = ? LIMIT 1",
            [renamed]
        );
    ...
    // Rename: update the name and any child references
    if (renamed && renamed !== trimmed) {
    ...
    const lookupName = renamed || trimmed;
    return tursoQueryOne(
        "SELECT name, parent_name, description, display_order, created_at, updated_at FROM wiki_categories WHERE name = ? LIMIT 1",
        [lookupName]
    );
