...
export default function AdminClient({ permissions }) {
  const tabs = [
    permissions.canEditWiki && "wiki",
    permissions.isStaff && "directory",
    permissions.isStaff && "characters",
  ].filter(Boolean);
    ...
    <main className="admin-shell">
      <header className="admin-header">
        <div>
          ...
          <p>
            Manage published knowledge, city directory presentation, and
            staff-only character lookup without writing to the game database.
          </p>
        </div>
        <span className="admin-lock">
          <i className="fa-regular fa-shield-halved" /> Role protected
        </span>
      </header>
      ...
      {tab === "wiki" && permissions.canEditWiki && (
        <WikiAdmin permissions={permissions} />
      )}
...
function WikiAdmin({ permissions }) {
  ...
  const load = async () => {
    try {
      setError(null);
      const res = await fetch("/api/wiki?admin=1", { cache: "no-store" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok || body.error) {
        throw new Error(body.error || `Wiki request failed (${res.status})`);
      }
      setData(body);
    } catch (err) {
      setError(err.message || "Failed to load wiki administration.");
    }
  };
  ...
  const grouped = useMemo(() => {
    ...
    for (const page of data?.pages || []) {
      (result[page.status] || result.draft).push(page);
    }
    return result;
  }, [data]);
  ...
  return (
    <section className="admin-panel">
      ...
      {data && (
        <>
          <div className="wiki-workflow-summary">
            <div>
              <strong>{grouped.draft.length}</strong>
              <span>Drafts</span>
            </div>
            <div>
          ...
          <div className="admin-wiki-list">
            {(["review", "draft", "published"]).map((status) => (
              <div key={status} className="admin-wiki-group">
                <h3>
                  {status === "review"
                    ? "Review queue"
                ...
                {grouped[status].length === 0 ? (
                  <p className="admin-empty-row">Nothing here.</p>
                ) : (
                  grouped[status].map((page) => (
                    <article key={page.slug} className="admin-wiki-row">
                      <div>
                      ...
                      <div className="admin-row-actions">
                        <Link href={`/wiki/${page.slug}/edit`}>Edit</Link>
                        {status === "draft" && (
                          <button
                            disabled={busy === page.slug}
                            onClick={() => setStatus(page, "review")}
                          >
                            Submit for review
                          </button>
                        )}
                        {status === "review" && permissions.canReviewWiki && (
                          <button
                            disabled={busy === page.slug}
...
function DirectoryAdmin() {
  ...
  const load = async () => {
    try {
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error || `Directory request failed (${res.status})`);
      setData(body);
    } catch (err) {
      setError(err.message || "Failed to load the City Directory admin data.");
    }
  };
  ...
  const metadataMap = useMemo(
    () => new Map((data?.metadata || []).map((entry) => [entry.job_key, entry])),
    [data]
  );
    ...
    <section className="admin-panel">
      ...
      {data && (
        <div className="directory-admin-layout">
          <div className="directory-admin-list">
            {(data.businesses || []).map((business) => (
              <button
                key={business.jobKey}
                className={selected?.jobKey === business.jobKey ? "active" : ""}
          ...
          <div>
            {selected ? (
              <form className="directory-admin-form" onSubmit={save}>
                <h3>{selected.name}</h3>
                <Field label="Display name" value={form.displayName} onChange={(value) => setForm({ ...form, displayName: value })} />
                <Field label="Description" value={form.description} multiline onChange={(value) => setForm({ ...form, description: value })} />
                <div className="admin-form-grid">
                  <Field label="Phone" value={form.phone} onChange={(value) => setForm({ ...form, phone: value })} />
                  <Field label="Hours" value={form.hours} onChange={(value) => setForm({ ...form, hours: value })} />
                </div>
                <Field label="Location" value={form.location} onChange={(value) => setForm({ ...form, location: value })} />
                <Field label="Logo URL" value={form.logoUrl} onChange={(value) => setForm({ ...form, logoUrl: value })} />
                <Field label="Services" help="Comma-separated" value={form.services} onChange={(value) => setForm({ ...form, services: value })} />
              ...
              <div className="admin-empty-card">
                ...
                <p>Select a listing to edit its website information.</p>
              </div>
            )}
          </div>
        </div>
...
function CharacterSearch() {
  ...
  async function search(event) {
    ...
    try {
      const res = await fetch(
        `/api/admin/characters?q=${encodeURIComponent(query.trim())}`,
        { cache: "no-store" }
      );
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error || `Character search failed (${res.status})`);
      setResults(body.characters || []);
      setSelected(null);
    } catch (err) {
      setResults([]);
      setSelected(null);
      setError(err.message || "Character search failed.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="admin-panel">
      <div className="admin-panel-head">
        <div>
          <span>Read-only game database</span>
          <h2>Character Search</h2>
        </div>
      </div>

      <form className="admin-character-search" onSubmit={search}>
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Name, Citizen ID, or phone"
        />
      ...
      <div className="admin-character-results">
        {loading ? (
          <p>Searching...</p>
        ) : (
          results.map((character) => (
            <button
              ...
              className={selected?.citizenid === character.citizenid ? "active" : ""}
            >
              <strong>{character.characterName}</strong>
              <span>
                {character.citizenid} · {character.employment?.label || "Unemployed"}
              </span>
            </button>
          ))
        )}
      ...
      {selected && (
        <div className="admin-character-preview">
          <CharacterCard character={selected} section="overview" />
        </div>
      )}
    </section>
  );
