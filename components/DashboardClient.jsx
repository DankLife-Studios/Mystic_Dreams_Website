  ...
  if (status === "unauthenticated") {
    return (
      <div className="dashboard-login">
        ...
        <h1 className="text-heading font-display mt-6 text-2xl font-semibold">Player dashboard</h1>
        <p className="text-body mt-2 text-sm leading-relaxed">
          Sign in with Discord to view whitelist status, characters, vehicles, licenses, and live server data.
        </p>
        <button
          type="button"
      ...
      <aside className="dashboard-sidebar">
        <div className="dashboard-profile">
          <div className="dashboard-profile-avatar">
            {user.image ? (
              <Image src={user.image} alt="" width={48} height={48} className="rounded-[0.625rem]" />
            ) : (
              <div className="dash-avatar-fallback">{displayName?.charAt(0) || "?"}</div>
            )}
            <span className={`dashboard-status-dot ${alerts.length === 0 ? "bg-emerald-500" : "bg-amber-500"}`} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-heading truncate font-semibold">{displayName}</p>
            {user.username && <p className="text-caption truncate text-xs">@{user.username}</p>}
          </div>
        </div>

        <div className="dashboard-status-list">
          <StatusRow icon="discord" label="Discord server" ok={discord.inGuild} okText="Joined" failText="Not joined" />
          <StatusRow icon="badge-check" label="Whitelist" ok={discord.hasCitizenRole} okText="Citizen" failText="Pending" />
          <StatusRow icon="link" label="Game account" ok={game.linked} okText="Linked" failText="Unlinked" />
        </div>

        {(permissions.isStaff || permissions.canEditWiki) && (
          <Link href="/admin" className="dashboard-staff-link">
            <Icon name="shield" size="xs" />
            <span>
              <strong>Staff tools</strong>
              <small>Wiki, directory & character lookup</small>
            </span>
        ...
        {alerts.map((alert) => (
          <div key={alert.key} className={`dashboard-alert dashboard-alert-${alert.tone}`}>
      ...
      <div className="dashboard-main">
        <header className="dashboard-main-header">
          ...
          <p className="text-body mt-2 max-w-2xl text-sm">
            Identity, contact details, employment, finances, licenses, last-synced status, and personal vehicles are read directly from your linked game account.
          </p>
        </header>

        {game.banned ? (
          <div className="dashboard-alert dashboard-alert-error">
            <p className="font-medium">{game.banReason || "Your account is restricted."}</p>
          </div>
        ) : game.characters?.length > 0 ? (
          <CharactersSection characters={game.characters} />
        ) : game.linked ? (
          <EmptyCharacterState linked />
        ) : (
          <EmptyCharacterState />
        )}
      </div>
...
function StatusRow({ icon, label, ok, okText, failText }) {
  return (
    <div className="dashboard-status-row">
      ...
      <span className={`dashboard-status-pill ${ok ? "dashboard-status-pill-ok" : "dashboard-status-pill-warn"}`}>
        {ok ? okText : failText}
      </span>
    </div>
  );
...
function buildAlerts(discord, game) {
  ...
  if (!discord.inGuild) {
    alerts.push({
      ...
      action: <DiscordButton className="!py-2 !text-xs" />,
    });
    ...
    alerts.push({
      ...
      action: (
        <Link href="/connect#whitelist" className="btn-secondary !py-2 !text-xs">
          Whitelist steps
        </Link>
      ),
    });
    ...
    alerts.push({
      ...
      action: (
        <Link href="/connect#fivem" className="btn-secondary !py-2 !text-xs">
          Connect guide
        </Link>
      ),
    });
